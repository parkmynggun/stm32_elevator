...
#endif
#define STEPS_PER_FLOOR 7450  // 예시값, 실제 값으로 조정하세요
...
{
    // 버튼 상태 읽기 (풀업, 눌리면 0)
    uint8_t btnDoorOpen = HAL_GPIO_ReadPin(GPIOC, GPIO_PIN_9);  // 문 열림 버튼 (PC9)
    uint8_t btnDoorClose = HAL_GPIO_ReadPin(GPIOB, GPIO_PIN_8); // 문 닫힘 버튼 (PB8)
